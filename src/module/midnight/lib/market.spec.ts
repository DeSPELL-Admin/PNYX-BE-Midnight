...
describe('market.ts', () => {
    ...
    describe('canonicalDatasetJson', () => {
        it('fixes key order v, tournamentId, orderId, rowCount, rows and auto-computes rowCount', () => {
            const json = canonicalDatasetJson({
                tournamentId: 7,
                orderId: 'abc123',
                rows: [
                    { tournamentId: 7, itemId: 1, bracket: [1, 2], segment: 'A', salt: 'saltA' },
                    { tournamentId: 7, itemId: 2, bracket: [1, 2], segment: 'B', salt: 'saltB' },
                ],
            });

            expect(json).toBe(
                '{"v":1,"tournamentId":7,"orderId":"abc123","rowCount":2,"rows":[{"tournamentId":7,"itemId":1,"bracket":[1,2],"segment":"A","salt":"saltA"},{"tournamentId":7,"itemId":2,"bracket":[1,2],"segment":"B","salt":"saltB"}]}',
            );
        });
        ...
        it('contains no newlines or extra whitespace', () => {
            const json = canonicalDatasetJson({
                tournamentId: 1,
                orderId: 'x',
                rows: [{ tournamentId: 1, itemId: 1, bracket: [1, 2], segment: 'A', salt: 's' }],
            });
